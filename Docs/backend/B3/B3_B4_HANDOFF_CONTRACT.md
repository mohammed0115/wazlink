...
A dedicated `BusinessUpdated` event — a B4-facing re-analysis trigger — is **not** emitted in Phase 1: no *B4* consumer exists yet, and inventing its payload before B4 is designed would be guessing at a contract B4 has not asked for. Recorded as **`B3-D-C010`**, and adding it later is purely additive.

**This is a different question from `BusinessUpserted`, and B3-FIX.1 corrects a prior conflation of the two.** Frozen `B2_CRM_LIST_QUERY_MODEL.md` (its `crm_lead_list_projection` refresh table) and `B2_FAILURE_SCENARIOS.md` (CF18) both name an event called **`BusinessUpserted`** as the mechanism that refreshes `business_name`, `business_category`, and `business_city` in CRM's list projection whenever a Business's normalized fields change after its first discovery. **B2 is therefore an existing, named consumer of a Business-changed signal — it is inaccurate to say no consumer exists for that class of event.** Frozen B0's event enumeration (`BACKEND_COMMAND_EVENT_CATALOG.md`) does not itself define `BusinessUpserted`; it is a name B2 assumed the Discovery side would supply, structurally identical to how B2 left `BusinessRediscovered`'s exact name to "be aligned during B3 Discovery Design" (`B2_REDISCOVERY_PROVENANCE_PROCESS.md` §2.1) — except that for `BusinessUpserted`, B2's text does not carry the same explicit "TBD" caveat.

B3's existing events do not fully cover it: `BusinessDiscovered` fires once, on first creation; `BusinessRediscovered` fires once per (Business, later job) pair; neither fires for a same-job re-observation or an in-place field refresh with no new discovering job. This gap is recorded honestly as **`B3-D-C019`** (`B3_DECISION_REGISTER.md` §4) rather than closed silently — it is deferred, not Class A, because `crm_lead_list_projection` is explicitly non-authoritative, rebuildable, and nightly-reconciled, and `GET /leads/{id}` / `/360` always read Business fields live (`B2_CRM_LIST_QUERY_MODEL.md` §6), so no authoritative correctness path in either domain depends on this event existing yet.
