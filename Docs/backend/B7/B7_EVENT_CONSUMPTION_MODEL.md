...
4. For each matching rule: evaluate conditions synchronously against the event payload plus any required `current.*` reads (§`B7_CONDITION_SNAPSHOT_SEMANTICS.md`); if matched, insert one `automation_runs` row (`status='queued'` or `'skipped'` if unmatched — a `skipped` run is still persisted, for the audit trail evidenced at FB-D15/FB-D19, not silently discarded).
...
- **Duplicate events:** the unique `(workspace_id, source_event_id)` constraint makes redelivery a safe no-op (§3-4).
...
- **Replayed events** (an operator manually replays a producing domain's outbox for recovery): identical to duplicate events — the dedup constraint absorbs it.
