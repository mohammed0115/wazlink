...
| Rule status | New triggers | Queued runs | Running runs | Waiting runs | Manual replay | Scheduled wakeups |
|---|---|---|---|---|---|---|
| `draft` | never admitted (rule was never `active`) | n/a | n/a | n/a | not permitted (`B7_RULE_LIFECYCLE.md` §3) | n/a |
| `active` | admitted normally | processed normally | run normally | resume normally | permitted | fire normally |
| `disabled` | **not admitted** — event still consumed/deduped (inbox record written), but no run is created for this rule | any run of this rule already `queued`/`awaiting_approval` at the moment of disabling is transitioned `cancelled` (nothing committed yet) | **unaffected** — finishes against its captured revision | **unaffected** — still wakes and resumes normally | permitted (replaying history is not "triggering," §`B7_DEAD_LETTER_REPLAY.md`) | fire normally (a wakeup belongs to an already-existing run, not to new admission) |
| `archived` | not admitted (rule cannot be `archived` while it was `active`; must be `disabled` first, `B7_RULE_LIFECYCLE.md` §2) | n/a (already resolved before archival was possible) | n/a | n/a | permitted | n/a |
...
**Which states can cancel:** `queued`, `evaluating`, `awaiting_approval`, `running`, `waiting` — any non-terminal state. Cancelling a `running` run does **not** interrupt whichever single action is currently mid-invocation (§4) — it prevents the *next* action from starting.
...
**Audit/event behavior:** `AutomationExecutionCancelled` fires once, listing exactly which actions had already reached a terminal state (`succeeded`/`failed`) versus which were skipped by the cancellation — full transparency, no silent gaps in the trail evidenced at FB-D15.
