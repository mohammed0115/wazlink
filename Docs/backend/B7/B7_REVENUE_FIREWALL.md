...
- **No action in the closed catalog targets B9.** `B7_ACTION_CATALOG.md` §2's ten Phase-1 actions target only B2 (5), B6 (1), B5 (1), and B7-internal control flow (2) — B9 does not appear as a `target_domain` value anywhere in the schema (`B7_DATA_MODEL.md` §4's `target_domain` column has no B9 case in the application code, because B9 does not exist yet to have one).
- **`create_revenue`/`create_attribution` are explicitly excluded**, not merely unimplemented (`B7_ACTION_CATALOG.md` §4) — named in `forbiddenAutomationActions` (FB-D06) and structurally impossible regardless of catalog content, since no B9 command exists anywhere in this corpus for any action to invoke.
