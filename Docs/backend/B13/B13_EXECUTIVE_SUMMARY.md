...
Eight external technical claims not already covered by a frozen phase's own research register were fetched from official documentation during this pass — Django's own security-settings warnings, **NIST SP 800-63B-4**'s password requirements (re-fetched under `B13-FIX.1` after the original pass cited the withdrawn 2020 edition, which had since been superseded — the correction raises the single-factor minimum length from 10 to 15 and makes compromised-password screening required rather than recommended), Sentry's data-scrubbing mechanics, OWASP's HTTP-headers guidance, and Celery's own pickle-serializer warning — six `VERIFIED`, one `PARTIAL` (an HSTS rollout cadence that is general practice rather than a specific Django recommendation, stated honestly as such), one `UNRESOLVED` and superseded by a successful re-fetch of the correct page (`B13_RESEARCH_REGISTER.md`). Every provider-specific fact already established by B5, B8, B10, or B12 (Meta's and Tap's webhook schemes, ZATCA's applicability model) is inherited by citation, never re-fetched or re-classified.

## 7. Self-verification

`B13_VERIFICATION_MATRIX.md` re-derives every counter the governing brief requires — 25 security controls, 28 threats across 16 actor classes, 139 concrete audit-event codes plus 5 non-enumerated audit requirements, held apart, 18 runbooks, 23 failure scenarios, 208 acceptance tests (43 positive, 165 negative) across 37 categories, every one of them reachable from the canonical register, and a clean reference-integrity pass. Three genuine defects were found and fixed during this authoring pass rather than hidden: two acceptance-test IDs (`AT-B13VAL`, `AT-B13CONC`) were cited before being defined and are now fully specified; `AT-B13CFG-1`/`2`/`3` were independently and inconsistently defined in two documents (one pair genuinely conflicting) and are now single-sourced; and three decisions were initially mislabeled as inherited "Class A" facts when they were B13's own original decisions, and have been renumbered into the Class B series. Reporting these as found-and-fixed, rather than presenting a falsely clean first draft, is the honest form this self-verification should take.

## 8. Package map

| Document | Purpose |
|---|---|
| `B13_FROZEN_INPUT_INVENTORY.md` | 96 frozen anchors from B0–B12 and the frontend, each with an exact citation |
...
| `B13_AUDIT_LOGGING.md` | consolidated security-audit catalog — 139 concrete codes and 5 non-enumerated requirements, distinct from the business timeline |
...
| `B13_SUPPLY_CHAIN_SECURITY.md` | dependency policy against the committed pnpm lockfile; records the vulnerability baseline as unrun, not remediated |
...
| `B13_ACCEPTANCE_TESTS.md` | canonical register of all 208 acceptance tests across 37 categories, each tagged P/N and attributed to its owning document |
| `B13_DECISION_REGISTER.md` | Class A (96, inherited) / B (29) / C (12) |
