...
- **Containment**: `DisableUser` on the affected account(s) — the frozen operator command, which revokes every session of that user with reason `user_disabled`; where the account must stay enabled, operator-targeted per-session revocation carries `admin_revoke`. Force a password reset, whose confirmation independently revokes all sessions. `RevokeAllSessions` is self-scoped in frozen B1 and is **not** an operator containment path.
