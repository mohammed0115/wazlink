...
Tap's `hashstring` HMAC-SHA256 field-concatenation signature (`FI-B8-01`) covers only the enumerated fields (`id`, `amount`, `currency`, `gateway_reference`, `payment_reference`, `status`, `created`) — **any field outside the concatenation is unauthenticated and must never drive an authorization or routing decision** (`FI-B12-02` §2). Combined with Tap's 3-attempt total retry bound (`B12-X-006`), a webhook-ingress outage during Tap's retry window is the single highest-priority payment-security operational risk in this pack (`B13_INCIDENT_MANAGEMENT.md` §2.4 "Payment authority failure").
...
The **conjunctive money-gate** (`FI-B9-02`, `B9-D-A038`), restated at frozen scope: **no B9 response** containing a `Money` field is reachable without `revenue.view`, whatever else the caller holds. The gate is B9-scoped by construction and `B13-FIX.1` restored that wording — a global "no response may carry Money without `revenue.view`" would contradict frozen B8, whose subscription/usage/invoice surfaces carry monetary values under `billing.view` (`B8_RBAC_TENANCY.md`) and never required `revenue.view`. B8 billing surfaces and B9 finance surfaces are separately gated; neither permission substitutes for the other, and holding one grants nothing on the other domain.
...
| `AT-B13FIN-4` | A **B9** response containing a `Money` field is unreachable without `revenue.view`, regardless of any other permission held; correspondingly, a B8 billing surface remains reachable under `billing.view` alone and does **not** require `revenue.view` |
