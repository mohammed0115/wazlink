...
| AI | `intelligence_runs` workspace-scoped, cache/reuse key is `(workspace_id, business_id, input_hash)` **never** `business_id` alone (prevents cross-workspace provider-cache bleed) | `signal_definitions`, `recommendation_definitions`, `scoring_model_versions` (catalog) | `FI-B4-02` |
