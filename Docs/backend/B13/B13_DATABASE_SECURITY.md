...
Distinct from the general database controls in §11: these verify the optimistic-concurrency mechanism itself (`version`/`If-Match`, `FI-B0-24` ADR-010), referenced from `B13_SECURITY_PRINCIPLES.md` control #12 and `B13_THREAT_MODEL.md`'s race-conditions row.
...
| `AT-B13CONC-2` | P | An update submitted without `If-Match`/`version` on a resource that requires it is rejected before any row lock is taken |
| `AT-B13CONC-3` | N | A successful versioned update's audit row carries both the before and after `version`, differing by exactly one |
