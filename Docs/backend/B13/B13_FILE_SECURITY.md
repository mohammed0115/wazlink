...
> Design only. Preserves `B11_SECURITY_PRIVACY.md`, `B11_RBAC_TENANCY.md`, `B11_FILE_VALIDATION.md`, `B11_STORAGE_KEY_MODEL.md`, `B11_CHECKSUM_INTEGRITY.md`, and `B11_DELETION_RETENTION_MODEL.md` verbatim (`FI-B11-01`…`FI-B11-05`). B13 adds no new file-security control; it states the production enforcement, monitoring, and incident-response layer.
...
| Ceiling | Value | Class |
|---|---|---|
| `MAX_FILE_BYTES` | 25 MiB (proposed) | `B13-D-B010`, Class B — deployment config, not plan-catalog data |
| `WORKSPACE_STORAGE_SAFETY_CEILING_BYTES` | 5 GiB (proposed), uniform across every plan | `B13-D-B011`, Class B — a platform safety floor, never an entitlement (`FI-B11-05`'s billing-boundary distinction: "an entitlement is something a customer can change by buying something; neither ceiling is") |
...
Soft-delete metadata, asynchronous physical purge; the `file_assets` row is the permanent tombstone — **no row is ever hard-deleted in Phase 1**, by any command, sweeper, or operator (`FI-B11-05`, `B11-D-A015`). `archived` has no exit transition — a provider-delete failure cannot resurrect access (`B11-D-A006`/`A016`). Retention timers (`UPLOAD_INTENT_TTL` ≈1h, `DOWNLOAD_TICKET_TTL` ≈5min, `ORPHAN_GRACE` ≈7 days, `PURGE_GRACE` ≈30 days) are all proposed Class B defaults (`B13-D-B014`), never applied to `legal`-class files, which are undeletable by any command/worker/timer/operator regardless of age (`FI-B11-05`).
