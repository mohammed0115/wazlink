...
| Stale worker writes | `outbox_events` (dispatch claim) and `worker_executions` (task execution) — two different frozen mechanisms | `outbox_events` **only**: per-claim `lease_token` fence, so a stale completion write matches zero rows (`B12-D-A055`, `B12_OUTBOX_MODEL.md` §3a). `worker_executions` has **no** lease or fencing column; a heartbeat-stale `running` row is classified `unknown` by reconciliation class `P-3`, never assumed failed (`FI-B12-07`) | `B13_REDIS_CELERY_SECURITY.md` §9 — B13 invents no lease, token, or fence column on `worker_executions` | heartbeat lapse past the job ceiling | stale outbox write discarded; a stale `worker_executions` row goes through the unknown-outcome procedure, never an automatic re-execution (`B12-D-A020`) | `AT-B13CEL-5` |
