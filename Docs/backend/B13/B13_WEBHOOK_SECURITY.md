...
> Design only. Preserves `B12_WEBHOOK_SECURITY.md` (`FI-B12-02`) and `B12_WEBHOOK_GATEWAY.md` (`FI-B12-17`, added under `B13-FIX.1` — the gateway document is load-bearing for the verify-before-anything ordering and the fail-closed rule for an unknown verification scheme, and `FI-B12-02`'s Source names only the security document) and every provider-specific verification document (`B5_WEBHOOK_SECURITY_MODEL.md`, `B8_TAP_PROVIDER_BOUNDARY.md`) verbatim. B13 does **not** invent a universal webhook verifier — B12 already proved that assumption wrong for this exact provider pair (`FI-B12-02`, `B12-D-A030`). This document is the operational and incident-response layer around the frozen verification design.
