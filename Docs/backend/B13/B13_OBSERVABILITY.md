...
| `backup_last_success_age_seconds` | *(new binding)* backup or WAL archiving stopped silently | page | Platform on-call | `B13_RUNBOOKS.md` §"Restore from backup"; control defined in `B13_BACKUP_RESTORE.md` §5a |
| `attribution_integrity_failure_total` | *(new binding)* impossible financial state | page, SEV-1 | Finance/Platform on-call | `B13_INCIDENT_MANAGEMENT.md` §2.5 "Financial corruption" |
...
| `security.rate_limited` / `security.credential_stuffing_suspected` rate | *(new binding)* authentication anomaly | page on sustained trigger | Security/Platform on-call | `B13_INCIDENT_MANAGEMENT.md` §2.2 "Authentication compromise" |
