...
Long operations use `POST → 202 Job resource → GET status/result`; provider callbacks are not exposed as trusted browser redirects. Analytics DTOs include workspace, period, timezone, currency, metric semantics, and snapshot/event classification. `Money.amount` is the decimal string `^-?\d+(\.\d{1,4})?$`; `Money.currency` is authoritative for a money value, and any transport-level currency mirror must equal it or the request is invalid. Report-level `currency` fields on aggregate DTOs declare the reporting scope and never override a stored `Money.currency`. Where a command references a stored server-priced resource — notably `PaymentCreate.quote_ref` naming an `UPQ-*` UpgradeQuote — the stored resource is the commercial source of truth: the server derives plan, amount, and currency from it, and any client-supplied commercial field is a non-authoritative mirror that must match or the request is rejected.
