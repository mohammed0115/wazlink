...
const journey = [
  ...
  ["06", "أغلق الصفقة", "Deal"],
  ["07", "قِس الإيراد المعترف به", "RevenueEvent / Analytics"],
];
      ...
      <main>
        <section className="premium-hero" id="platform">
          <div className="premium-hero-copy">
            ...
            <p>WazLink تجمع اكتشاف العملاء، CRM، واتساب والذكاء الاصطناعي في منصة واحدة تدير رحلة المبيعات من أول فرصة حتى قياس الإيراد المعترف به.</p><small className="landing-disclosure">لا تقرر الباقة الآن؛ ابدأ بالاكتشاف، وستظهر خيارات Excel أو CRM بعد النتائج المكتملة.</small>
          ...
          <div className="hero-product-frame" aria-label="معاينة لوحة WazLink">
            <div className="product-browser-bar"><i /><i /><i /><span>app.wazlink.ai</span><small>معاينة توضيحية · ليست بيانات حساب حقيقية</small></div>
            <div className="product-dashboard-preview">
              <aside className="preview-sidebar"><div className="preview-brand"><img src={`${appConfig.assetBaseUrl}wazlink-mark.svg`} alt="" /><b>WazLink</b></div><span className="active">⌂ لوحة التحكم</span><span>◉ واتساب AI</span><span>⌖ سحب بيانات تجريبية</span><span>♙ CRM العملاء</span><span>◫ المتابعة</span><span>▥ تقارير توضيحية</span><small>منصة المبيعات الذكية</small></aside>
              <div className="preview-workspace"><header><span>الرئيسية</span><b>مرحبًا، أحمد</b><i>◔</i></header><div className="preview-welcome"><div><small>لوحة القيادة التنفيذية</small><h3>كل فرصك في مكان واحد</h3><p>من البحث إلى المحادثة ثم الصفقة.</p></div><div className="mini-map"><i>●</i><i>●</i><i>●</i><b>Google Maps</b></div></div><div className="preview-kpis" aria-label="معاينة توضيحية وليست بيانات حساب حقيقية"><div><span>العملاء المحتملون</span><b>بيانات تجريبية</b><small>معاينة توضيحية فقط</small></div><div><span>المحادثات</span><b>بيانات تجريبية</b><small>لا تمثل معدلًا حقيقيًا</small></div><div><span>الصفقات</span><b>بيانات تجريبية</b><small>ليست قيمة Pipeline فعلية</small></div><div><span>الإيراد المعترف به</span><b>منفصل</b><small>يظهر من RevenueEvent فقط</small></div></div><div className="preview-lower"><div className="preview-chart"><div><b>أداء المبيعات</b><span>آخر 30 يومًا</span></div><svg viewBox="0 0 360 100" role="img" aria-label="رسم أداء المبيعات"><path d="M0 82 C40 65,50 78,80 58 S130 75,160 42 S210 55,240 28 S300 54,360 12" fill="none" stroke="#2563eb" strokeWidth="4" /><path d="M0 82 C40 65,50 78,80 58 S130 75,160 42 S210 55,240 28 S300 54,360 12 V100 H0Z" fill="url(#fill)" opacity=".2" /><defs><linearGradient id="fill" x1="0" x2="0" y1="0" y2="1"><stop stopColor="#2563eb" /><stop offset="1" stopColor="#fff" stopOpacity="0" /></linearGradient></defs></svg></div><div className="preview-conversations"><b>محادثات واتساب الأخيرة</b><span>شركة النخبة <small>متابعة جديدة</small></span><span>مطعم الرياض <small>بانتظار الرد</small></span><span>عيادة الحياة <small>فرصة عالية</small></span></div></div></div>
            </div>
          </div>
        </section>

        <section className="value-strip" aria-label="قدرات المنصة وليست إحصاءات مستخدمين"><div><b>محلي</b><span>سجلات قابلة للمتابعة</span></div><div><b>متصل</b><span>رحلة من الاكتشاف إلى CRM</span></div><div><b>بشري</b><span>المتابعة والإرسال بقرار الفريق</span></div><div><b>قابل للقياس</b><span>الإيراد المعترف به من RevenueEvent</span></div></section>

        <section className="premium-section modules-section" id="uses"><div className="section-heading centered"><span className="premium-badge">منصة واحدة، نتائج متصلة</span><h2>كل ما تحتاجه لتنمية مبيعاتك في مكان واحد</h2><p>ثلاث قدرات قوية تعمل كرحلة واحدة — من اكتشاف الإشارة إلى قياس الإيراد المعترف به.</p></div><div className="premium-module-grid"><article className="premium-module-card discover"><div className="module-visual maps-visual"><span>⌖</span><div><b>عيادات أسنان</b><small>الرياض · معاينة توضيحية</small></div><i>—</i></div><small className="module-label">WazLink Discover</small><h3>اكتشاف العملاء المحتملين</h3><p>ابحث عن الشركات والعملاء المحتملين وحوّل بيانات السوق إلى فرص مبيعات قابلة للمتابعة.</p><button className="text-link" type="button" onClick={() => go("discovery")}>ابدأ الاكتشاف ←</button></article><article className="premium-module-card crm"><div className="module-visual kanban-visual"><span>جديد</span><span>مؤهل</span><span>صفقة</span></div><small className="module-label">WazLink CRM</small><h3>إدارة العملاء ومسار المبيعات</h3><p>حوّل النتائج إلى Leads، وزّعها على فريقك، وتابع الفرص والصفقات من مكان واحد.</p><button className="text-link" type="button" onClick={() => go("crm")}>افتح CRM ←</button></article><article className="premium-module-card ai"><div className="module-visual ai-visual"><span>واتساب</span><b>أرسل متابعة قصيرة تتضمن رابط الحجز</b><small>مقترح من مساعد المبيعات · مراجعة بشرية</small></div><small className="module-label">WazLink AI</small><h3>واتساب ومساعد المبيعات الذكي</h3><p>اجمع المحادثات في Inbox واحد واستخدم الذكاء الاصطناعي لفهم السياق واقتراح الخطوة التالية.</p><button className="text-link" type="button" onClick={() => go("inbox")}>افتح صندوق الوارد ←</button></article></div></section>

        <section className="premium-section journey-section" id="workflow"><div className="section-heading centered"><span className="premium-badge">رحلة مبيعات واحدة</span><h2>من أول بحث إلى قياس الإيراد المعترف به</h2><p>كل خطوة تحمل سياقها وتمنح فريقك إجراءً واضحًا بعدها.</p></div><div className="premium-journey">{journey.map(([number, title, detail], index) => <div className="journey-step" key={number}><span>{number}</span><b>{title}</b><small>{detail}</small>{index < journey.length - 1 ? <i>←</i> : null}</div>)}</div></section>

        <section className="premium-section showcase-section"><div className="showcase-copy"><span className="premium-badge">WazLink Discover</span><h2>لا تبحث عن أسماء فقط<br /><em>ابحث عن فرص بيع</em></h2><p>كل شركة تظهر كسجل قابل للفهم، مع درجة الفرصة والأدلة والإجراء التالي — لا كسطر معزول في ملف.</p><button className="button primary" type="button" onClick={() => go("discovery")}>ابدأ الاكتشاف</button></div><div className="discovery-product-card"><div className="search-bar">⌕ <span>عيادات أسنان | الرياض</span><b>بحث</b></div><div className="business-result"><div className="result-top"><div className="result-avatar">ع</div><div><b>{businessName}</b><small>⌖ {businessCity} · Google Maps</small></div><strong>{arabicNumber(score)}<small>/100</small></strong></div><div className="result-stats"><span>التقييم <b>4.7</b></span><span>المراجعات <b>863</b></span><span>الثقة <b>{arabicNumber(confidence)}%</b></span><span>واتساب <b>متاح</b></span><span>الموقع <b>يحتاج تحديثًا</b></span></div><div className="signal-list">{businessReasons.map((reason) => <span key={reason}>✓ {reason}</span>)}</div><div className="opportunity-row"><small>الفرصة المقترحة</small><b>{businessServices}</b></div><button type="button" onClick={() => go("discovery")}>راجع النتائج ←</button></div></div></section>

        <section className="premium-section context-section"><div className="section-heading centered"><span className="premium-badge">سياق متصل</span><h2>لا تفقد سياق العميل بعد الاكتشاف</h2><p>تتحرك نفس الهوية من الشركة إلى العميل المحتمل والمحادثة والصفقة، بينما يبقى الإيراد المعترف به مصدرًا منفصلًا.</p></div><div className="context-flow"><div><span>⌖</span><b>شركة</b><small>إشارة ومصدر</small></div><i>←</i><div><span>♙</span><b>عميل محتمل</b><small>درجة ومالك</small></div><i>←</i><div><span>◌</span><b>محادثة</b><small>سياق ومتابعة</small></div><i>←</i><div><span>◇</span><b>صفقة</b><small>قيمة واحتمال</small></div><i>←</i><div className="revenue-node"><span>ر.س</span><b>إيراد معترف به</b><small>RevenueEvent فقط</small></div></div></section>
        ...
        <section className="premium-section analytics-section"><div className="section-heading"><span className="premium-badge">WazLink Analytics</span><h2>اعرف أين تُقاس الفرص كإيراد معترف به</h2><p>قراراتك مبنية على القمع، مسار المبيعات، ومصدر العميل؛ وتبقى RevenueEvent منفصلة عن قيمة الصفقة.</p></div><div className="analytics-product-card"><div className="analytics-funnel">{[["مكتشف", stageCount("discovered", 1248)], ["مؤهل", stageCount("high", 420)], ["CRM Leads", stageCount("lead", 310)], ["تم التواصل", "١٨٥"], ["صفقة", stageCount("deal", 48)], ["رابحة", stageCount("won", 17)]].map(([label, value], index) => <div key={label} style={{"--funnel-width": `${100 - index * 10}%`} as CSSProperties}><i /><b>{value}</b><span>{label}</span></div>)}</div><div className="analytics-summary"><div><span>الإيراد المعترف به · من RevenueEvent</span><b>{sarLabel(metrics.revenue.value)}</b></div><div><span>مسار المبيعات · قيمة صفقات مفتوحة</span><b>{sarLabel(metrics.openPipeline.value)}</b></div><div><span>مصدر توضيحي</span><b>بيانات محلية</b></div></div></div></section>
